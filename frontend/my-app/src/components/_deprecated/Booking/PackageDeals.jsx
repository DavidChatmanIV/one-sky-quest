...
/* ---------- Images ---------- */
/* Swap these to your local assets when ready */
const CITY_IMAGES = {
  MIA: "https://source.unsplash.com/1200x800/?miami,beach",
  LAS: "https://source.unsplash.com/1200x800/?las%20vegas,strip",
};
const HOTEL_IMAGES = {
  "Seabreeze Resort": "https://source.unsplash.com/1200x800/?beach,resort",
  "Downtown Loft Hotel":
    "https://source.unsplash.com/1200x800/?miami,downtown,hotel",
};
function getPkgImage(pkg) {
  return (
    HOTEL_IMAGES[pkg?.hotel?.name] ||
    CITY_IMAGES[pkg?.flight?.to || pkg?.hotel?.city || "MIA"] ||
    "https://source.unsplash.com/1200x800/?travel,city"
  );
}
...
function buildPackages(criteria) {
  ...
  flightChoices.forEach((f) => {
    hotelChoices.forEach((h) => {
      ...
      const baseDiscount = f.nonstop ? 0.12 : 0.08;
      const bundlePrice = Math.round(separate * (1 - baseDiscount));
      const includes = ["Round-trip flights", `${nights}-night stay`];

      if (bundleType === "flight_hotel_car") {
        carChoices.forEach((c) => {
          const separateWithCar = separate + c.pricePerDay * days;
          const carDiscount = 0.15;
      ...
      if (bundleType === "hotel_car") {
        carChoices.forEach((c) => {
          ...
          const price = Math.round(hotelCarSeparate * 0.88);
          results.push({
            id: `${h.name}-${c.vendor}`,
    ...
    return [
      {
        ...
        crossed: 1180,
        savings: 181,
        flight: { airline: "Any", nonstop: true, to: "MIA" },
        hotel: { name: "Seabreeze Resort", city: "MIA" },
        car: null,
      ...
      >
        <Row align="middle" style={{ marginBottom: 8 }}>
          <Col flex="auto">
            <Title
              level={4}
              style={{ color: "var(--text, #f4f6fb)", margin: 0 }}
            >
              Package Deals
            </Title>
          ...
          {data.map((pkg) => (
            <Col xs={24} md={12} lg={8} key={pkg.id}>
                ...
                cover={
                  <div className="pkg-media">
                    ...
                    <div className="pkg-ribbon">
                      ...
                      {pkg.hotelRating ? (
                        <Tag className="pkg-chip pkg-chip--rating">
                          {pkg.hotelRating.toFixed(1)}★
                        </Tag>
                      ) : null}
                      <Tag className="pkg-chip pkg-chip--xp">
                        <ThunderboltOutlined /> XP +200
                      </Tag>
                    </div>
                  </div>
                }
              >
                ...
                <Title
                  level={5}
                  style={{ marginTop: 2, color: "var(--text, #fff)" }}
                >
                  {pkg.title}
                </Title>
